...
int BPU_gf2QcMatrixToSparse(BPU_T_GF2_Sparse_Qc_Matrix *out, const BPU_T_GF2_QC_Matrix *in, const int wi[]) {
  ...
  BPU_gf2SparseQcMatrixMalloc(out, in->row_element_count, in->element_size, 1);
...
void BPU_gf2PolyTransp(BPU_T_GF2_Poly *out, const BPU_T_GF2_Poly *in) {
  ...
  if (BPU_gf2VecGetBit(in, 0)) {
    BPU_gf2VecSetBit(out, 0, 1);
  }

  // swap other coefficients
  for (i = 1; i < out->len; i++) {
    if (BPU_gf2VecGetBit(in, i)) {
      BPU_gf2VecSetBit(out, out->len - i, 1);
    }
  }
}
...
void BPU_gf2QcPermuteMatrixColumns(BPU_T_GF2_QC_Matrix *out, BPU_T_Perm_Vector *permutation) {
  int permutation_track[permutation->size];
  int i, j, p, t;
  BPU_T_GF2_Poly temp;
  for (i = 0; i < permutation->size; i++) {
...
void BPU_gf2QcMatrixGetColumn(BPU_T_GF2_Vector *out, BPU_T_GF2_QC_Matrix *in, int column) {
  int i, j, block, position, offset;
  BPU_T_GF2_Poly transp, *source;
  ...
  for (i = 0; i < in->column_element_count; i++) {
    source = &in->matrices[i * in->row_element_count + block];

    if (BPU_gf2PolyIsZero(source)) {
      continue;
    }

    BPU_gf2PolyTransp(&transp, source);
    offset = i * in->element_size;
    ...
    for (j = 0; j < transp.len; j++) {
      if (BPU_gf2VecGetBit(&transp, j)) {
        BPU_gf2VecSetBit(out, offset + j, 1);
      }
    }

    BPU_gf2PolyReset(&transp);
  }
...
void BPU_gf2QcMatrixGetRow(BPU_T_GF2_Vector *out, BPU_T_GF2_QC_Matrix *in, int row) {
  int i, j, block, position, offset;
  BPU_T_GF2_Poly block_poly, *source;
  ...
  for (i = 0; i < in->row_element_count; i++) {
    source = &in->matrices[block * in->row_element_count + i];

    if (BPU_gf2PolyIsZero(source)) {
      continue;
    }

    BPU_gf2PolyCopy(&block_poly, source);
    ...
    for (j = 0; j < block_poly.len; j++) {
      if (BPU_gf2VecGetBit(&block_poly, j)) {
        BPU_gf2VecSetBit(out, i * block_poly.len + j, 1);
      }
    }
    BPU_gf2PolyReset(&block_poly);
  }
