...
/**
 * Transpose QC matrix with more than more row
 *
 * @param out   output of the transposition
 * @param in    input matrix
 */
void BPU_gf2QcMultirowMatrixTransp(BPU_T_GF2_QC_Matrix *out, const BPU_T_GF2_QC_Matrix *in);

/**
 * Allocates the QC identity matrix of the given size
 *
 * @param out         ouput, identity matrix
 * @param block_size  size of the circulant block
 * @param block_count number of blocks in rows and columns
 */
...
/**
 * Creates a QC selection of the matrix.
 *
 * @param out           ouput of the selection
 * @param in            input matrix
 * @param column_start  column block in which the selection starts
 * @param column_end    column block in which the selection ends
 * @param row_start     row block in which the selection starts
 * @param row_end       row block in which the selection ends
 */
...
/**
 * Multiplies two QC matrices together
 *
 * @param out           ouput of the multiplication
 * @param left          left side of the multiplication
 * @param right         right side of the multiplication
 */
void BPU_gf2QcMatrixMultiply(BPU_T_GF2_QC_Matrix *out, BPU_T_GF2_QC_Matrix *left, BPU_T_GF2_QC_Matrix *right);

/**
 * Adds two QC matrices together. out = out + in
 *
 * @param out           ouput of the addition
 * @param in            matrix to add
 */
void BPU_gf2QcMatrixAdd(BPU_T_GF2_QC_Matrix *out, BPU_T_GF2_QC_Matrix *in);

/**
 * Function to calculate inverse of the matrix. Return value is 0 or 1.
 * 0 if the inversion was not found, 1 if it was successfull
 *
 * @param out           inverse of the input (if the inversion exists)
 * @param in            matrix to invert
 */
int BPU_gf2QcInvMatrix(BPU_T_GF2_QC_Matrix *out, BPU_T_GF2_QC_Matrix *in);

/**
 * Tries to transform the input matrix to reduced row echelon form.
 * Returns 1 if successfull. Only if on left side is identity matrix
 * This is used to calculate inverse of the matrix. It is discouraged to
 * use it for different purposes.
 *
 * @param out           ouput of the transformation
 * @param in            matrix to transform to RREF
 */
int BPU_gf2QcMatrixToRref(BPU_T_GF2_QC_Matrix *out, BPU_T_GF2_QC_Matrix *in);

/**
 * Swaps two row blocks with each other
 *
 * @param in            input matrix in which the swap occurs
...
/**
 * Copy content of input matrix to output
 *
 * @param out           copy of the input
 * @param input
 */
void BPU_gf2QcMatrixCopy(BPU_T_GF2_QC_Matrix *out, BPU_T_GF2_QC_Matrix *in);

/**
 * Multiplies row block with a block element
 *
 * @param out           ouput of the multiplication
 * @param row           row block to multiply
 * @param multiple      multiple with which is the matrix row multiplied
 */
void BPU_gf2QcMatrixMultiplyRow(BPU_T_GF2_QC_Matrix *out, int row, BPU_T_GF2_Poly *multiple);

/**
 * Adds multiple of the row to another row (used in the GEM).
 *
 * @param out           matrix in which the operation takes place
 * @param source        row block which multiple is added to target
 * @param target        row block to which the multiple is added
 * @param multiple      multiple with which is the matrix row multiplied
 * @param module        module with which the matrix blocks are reduced
 */
...
/**
 * Permutes the column blocks with given permutation
 *
 * @param out           matrix which columns are permuted
 * @param permutation   permutation which is applied to the column blocks
 */
 ...
 * Permutes the column blocks with given permutation
 *
 * @param out           matrix which rows are permuted
 * @param permutation   permutation which is applied to the rows blocks
 */
void BPU_gf2QcPermuteMatrixRows(BPU_T_GF2_QC_Matrix *out, BPU_T_Perm_Vector *permutation);

/**
 * Multiplies matrix with a vector out = mat*vec
 *
 * @param out           result of the multitplication
 * @param mat           left side of the multiplication
 * @param vec           right side of the multiplication
 */
void BPU_gf2QcMatrixVectorMultiply(BPU_T_GF2_Vector *out, BPU_T_GF2_QC_Matrix *mat, BPU_T_GF2_Vector *vec);

/**
 * Gets column of the QC matrix as a simple vector
 *
 * @param out           matrix column
 * @param in            input matrix
...
/**
 * Gets row of the QC matrix as a simple vector
 *
 * @param out           matrix row
 * @param in            input matrix
 * @param column        row to get
 */
